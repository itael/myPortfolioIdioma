...
const Navbar = () => {
  ...
  return (
    <header className='shadow-md w-full fixed top-0 left-0 z-20'>
      <nav className='flex items-center justify-between bg-white py-4 nr:py-3 nr:px-10 px-7 dark:bg-gray-800 transition-all duration-250 ease-in'>
        ...
        <ul className={`shadow-md nr:shadow-none dark:bg-gray-700 dark:nr:bg-gray-800 nr:flex nr:items-center md:pb-0 pb-6 md:pt-0 pt-2 absolute nr:static bg-white nr:z-auto z-[-1] left-0 w-full nr:w-auto nr:pl-0 pl-9 transition-all duration-250 ease-in ${open ? 'top-16' : 'top-[-490px]'}`}>
          {
            Links.map((link) => (
              <li key={link.name} className='nr:ml-7 text-lg nr:my-0 my-4'>
                <a href={link.link} onClick={() => setOpen(!open)} className='text-gray-800 dark:text-white dark:hover:text-gray-400 hover:text-gray-400 duration-500 '>
                  <code className='text-base'>{`<${link.name}/>`}</code>
                </a>
              </li>
            ))
          }
          <LanguageButton/>
          {
            SocialMedia.map((link) => (
              <li key={link.name} className='md:hidden md:ml-7 md:my-0 my-4'>
